...
atribuir(100, des=0.1, gar= 20, imp= 0.01)

def listar(*listas):
    alll = []
    for lista in listas:
        for item, produto in lista:
            alll.append(item)
        delete = set(alll)
        reverse = list(delete)
    return reverse
...
print(listar(estoque_fim_jan, estoque_fim_fev, estoque_fim_mar, estoque_fim_abr))
print(len(listar(estoque_fim_jan, estoque_fim_fev, estoque_fim_mar, estoque_fim_abr)))